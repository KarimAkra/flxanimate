...
@:final class BlurFilter extends FlxAnimateFilter
{
  ...
  public override function initShaderGraphic(?pass:Int, ?sourceBitmapData:BitmapData):GraphicsShader
  {
    #if !macro
		if (pass < __horizontalPasses)
		{
    ...
    {
      __blurShader.bitmap.input = sourceBitmapData;
  		__blurShader.bitmap.filter = __smooth ? LINEAR : NEAREST;
    }
		#end
...
private class BlurShader extends GraphicsShader
{
	@:glFragmentSource("
 		#pragma header
      ...
      #pragma body
			...
			vec4 sum = vec4(0.0);
			sum += texture2D(bitmap, vBlurCoords[0]) * 0.00443;
			sum += texture2D(bitmap, vBlurCoords[1]) * 0.05399;
			sum += texture2D(bitmap, vBlurCoords[2]) * 0.24197;
			sum += texture2D(bitmap, vBlurCoords[3]) * 0.39894;
			sum += texture2D(bitmap, vBlurCoords[4]) * 0.24197;
			sum += texture2D(bitmap, vBlurCoords[5]) * 0.05399;
			sum += texture2D(bitmap, vBlurCoords[6]) * 0.00443;
